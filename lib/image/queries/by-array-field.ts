...
import { DEFAULT_LIMIT, getCollection } from './base';
...
export async function getImagesByFeeling(
  ...
  return col.find({ feelings: feeling }).sort({ _id: -1 }).limit(limit).toArray();
}
...
export async function getImagesByHue(
  ...
  return col.find({ hues: hue }).sort({ _id: -1 }).limit(limit).toArray();
}
...
export async function getImagesByExactColor(
  ...
  return col.find({ colors: normalizedColor }).sort({ _id: -1 }).limit(limit).toArray();
}
